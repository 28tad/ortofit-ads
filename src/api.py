...
ВАЖНО: у аккаунта сейчас право только на чтение. Методы записи
(add/update/suspend/resume) здесь реализованы, но Директ отклонит их,
пока право на запись не выдано. Вызываются они только из-под флага --apply.
"""
...
class DirectApi:
    ...
    # --- запись ----------------------------------------------------------
    # Ниже методы, которые Директ пока отклоняет: право на запись не выдано.
    # Код держим готовым, чтобы включение доступа не требовало правок.
